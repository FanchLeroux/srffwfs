...
print(os.environ.get("OOPAO_BACKEND", "Not set"))
...
tel = Telescope(
    resolution=6 * n_subaperture,  # resolution of the telescope in [pix]
...
ngs = Source(
    optBand="Na",  # Optical band (see photometry.py)
    magnitude=8,  # Source Magnitude
    coordinates=[0, 0],
)  # Source coordinated [arcsec,deg]
...
src = Source(
    optBand="K",  # Optical band (see photometry.py)
    magnitude=8,  # Source Magnitude
    coordinates=[1, 0],
)  # Source coordinated [arcsec,deg]
...
cam = Detector(
    integrationTime=tel.samplingTime,  # integration time of the detector
...
dm = DeformableMirror(
    telescope=tel,  # Telescope
    nSubap=nAct
    - 1,  # number of subaperture of the system considered (by default the DM has n_subaperture + 1 actuators to be in a Fried Geometry)
    ...
    pitch=tel.D / nAct,
)  # inter actuator distance. Only used to compute the influence function coupling. The default is based on the n_subaperture value.
...
wfs = Pyramid(
    nSubap=n_subaperture,  # number of subaperture = number of pixel accros the pupil diameter
...
)  # slopesMap_incidence_flux, fullFrame_incidence_flux (see documentation)
...
ngs.nPhoton = (
    n_photons_per_subap / tel.samplingTime / (surface_telescope / n_valid_subap)
...
M2C_KL = compute_KL_basis(tel, atm, dm, lim=0)  # inversion stability criterion
...
calib_modal = InteractionMatrix(
    ...
    single_pass=True,
)  # only push to compute the interaction matrix instead of push-pull
...
plot_obj = cl_plot(
    list_fig=[
        atm.OPD,  # list of data for the different subplots
        tel.OPD,
        tel.OPD,
        wfs.cam.frame,
    ...
    type_fig=[
        "imshow",  # type of figure for the different subplots
        "imshow",
        "imshow",
        "imshow",
        "imshow",
    ...
    list_title=[
        "Turbulence [nm]",  # list of title for the different subplots
        "NGS residual [m]",
        "SRC residual [m]",
        "WFS Detector",
    ...
    list_legend=[
        None,  # list of legend labels for the subplots
        None,
        None,
        None,
        None,
        [
            ...
            "NGS@" + str(ngs.coordinates[0]) + '"',
        ],
        None,
        None,
    ],
    list_label=[
        None,  # list of axis labels for the subplots
        None,
        None,
        None,
        None,
        ...
        [
            "NGS PSF@"
            + str(ngs.coordinates[0])
            + '" -- FOV: '
            + str(np.round(ngs_cam.fov_arcsec, 2))
            + '"',
            "",
        ],
        [
            "SRC PSF@"
            + str(src.coordinates[0])
            + '" -- FOV: '
            + str(np.round(src_cam.fov_arcsec, 2))
            + '"',
            "",
        ],
    ],
    n_subplot=[4, 2],
    list_display_axis=[
        None,  # list of the subplot for which axis are displayed
        None,
        None,
        None,
        None,
    ...
    list_ratio=[[0.95, 0.95, 0.1], [1, 1, 1, 1]],
    s=20,
...
for i in range(nLoop):
    ...
    SR_ngs[i] = strehlMeter(
        PSF=ngs_cam.frame, tel=tel, PSF_ref=ngs_psf_ref, display=False
    )
    ...
    # save Strehl ratio from the PSF image
    SR_src[i] = strehlMeter(
        PSF=src_cam.frame, tel=tel, PSF_ref=src_psf_ref, display=False
    )
    ...
    if display and i > 1:
        ...
        # update title
        plot_obj.list_title = [
            "Turbulence " + str(np.round(wfe_atmosphere[i])) + "[nm]",
        ...
        cl_plot(
            list_fig=[
                ...
                [
                    np.arange(i + 1),
                    wfe_residual_SRC[: i + 1],
                    wfe_residual_NGS[: i + 1],
                ],
                NGS_PSF,
                SRC_PSF,
            ],
    ...
    print(
        ...
        np.round(SR_ngs[i], 1),
        " WFE [nm] : ",
        np.round(wfe_residual_NGS[i], 2),
    )
    print(
        "SRC: Strehl ratio [%] : ",
        np.round(SR_src[i], 1),
        " WFE [nm] : ",
        np.round(wfe_residual_SRC[i], 2),
    )
...
plt.figure()
plt.plot(
    np.arange(nLoop) * tel.samplingTime,
    SR_ngs,
    label="NGS@" + str(np.round(1e9 * ngs.wavelength, 0)) + " nm",
)
plt.plot(
    np.arange(nLoop) * tel.samplingTime,
    SR_src,
    label="SRC@" + str(np.round(1e9 * src.wavelength, 0)) + " nm",
)
plt.legend()
plt.xlabel("Time [s]")
plt.ylabel("SR [%]")
