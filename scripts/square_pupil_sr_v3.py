...
modes_weight_2g = (np.abs(vt2g[:n_modes_2g]) ** 2).sum(axis=0)

first_modes_2g = np.argsort(modes_weight_2g)[-n_modes_2g:][::-1]

plt.figure()
plt.plot(modes_weight_2g)

print(f"First {n_modes_2g} modes: {first_modes_2g}")

first_modes_2g_labels = [labels_sr[i] for i in first_modes_2g]
draw_labels(labels_sr)
draw_labels(first_modes_2g_labels)
